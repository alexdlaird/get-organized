...
/**
 * This is a panel that offers a robust set of controls to pick a color. <P>This
 * was originally intended to replace the
 * <code>JColorChooser</code>. To use this class to create a color choosing
 * dialog, simply call:
 * <BR><code>ColorPicker.showDialog(frame, originalColor);</code> <P>However
 * this panel is also resizable, and it can exist in other contexts. For
 ...
 * <BR><code>picker.setMode(ColorPicker.HUE);</code> <P>This will create a
 * miniature color picker that still lets the user choose from every available
 * color, but it does not include all the buttons and numeric controls on the
 * right side of the panel. This might be ideal if you are working with limited
 * space, or non-power-users who don't need the RGB values of a color. The
 * <code>main()</code> method of this class demonstrates possible ways you can
 * customize a
 * <code>ColorPicker</code> component. <P>To listen to color changes to this
 * panel, you can add a
 ...
 * RGB value of the selected color changes. <P>To listen to opacity changes to
 * this panel, use a
 * <code>PropertyChangeListener</code> listening for changes to the
...
public final class ColorPicker extends JPanel
{
    ...
    /**
     * This creates a modal dialog prompting the user to select a color. <P>This
     * uses a generic dialog title: "Choose a Color", and does not include
     * opacity.
     *
     * @param owner the dialog this new dialog belongs to. This must be a Frame
     * or a Dialog. Java 1.6 supports Windows here, but this package is
     * designed/compiled to work in Java 1.4, so
     * an <code>IllegalArgumentException</code> will be thrown if this component
     * is a <code>Window</code>.
     * @param originalColor the color the <code>ColorPicker</code> initially
     * points to.
    ...
    /**
     * This creates a modal dialog prompting the user to select a color. <P>This
     * uses a generic dialog title: "Choose a Color".
     *
     * @param owner the dialog this new dialog belongs to. This must be a Frame
     * or a Dialog. Java 1.6 supports Windows here, but this package is
     * designed/compiled to work in Java 1.4, so
     ...
     * @param originalColor the color the <code>ColorPicker</code> initially
     * points to.
     * @param includeOpacity whether to add a control for the opacity of the
     * color.
     * @return the <code>Color</code> the user chooses, or <code>null</code> if
     * the user cancels the dialog.
     */
     ...
     * the selected color changes. <P>(Events are only created when then RGB
     * values of the color change. This means, for example, that the change from
     * HSB(0,0,0) to HSB(.4,0,0) will <i>not</i> generate events, because when
     * the brightness stays zero the RGB color remains (0,0,0). So although the
     * hue moved around, the color is still black, so no events are created.)
     *
     */
    ...
    /**
     * Create a new
     * <code>ColorPicker</code> with all controls visible except opacity.
     */
    public ColorPicker()
    {
        this (true, false);
     ...
     *
     * @param showExpertControls the labels/spinners/buttons on the right side
     * of a <code>ColorPicker</code> are optional. This boolean will control
     * whether they are shown or not. <P>It may be that your users will never
     * need or want numeric control when they choose their colors, so hiding
     * this may simplify your interface.
     ...
     * <P>Note this lives inside the "expert controls", so if
    ...
    /**
     * This controls whether the preview swatch visible or not. <P>Note this
     * lives inside the "expert controls", so if
     * <code>setExpertControlsVisible(false)</code> has been called, then
     ...
     * they are shown or not. <P>It may be that your users will never need or
     * want numeric control when they choose their colors, so hiding this may
     ...
     * <code>ColorPicker</code>. This is especially useful if this picker is in
     * non-expert mode, so the radio buttons are not visible for the user to
     * directly select.
     *
     ...
     * <P>Note these live inside the "expert controls", so if
    ...
    /**
     * @return the current mode of this <code>ColorPicker</code>. <BR>This will
     * return <code>HUE</code>,  <code>SAT</code>,  <code>BRI</code>,
     * <code>RED</code>,  <code>GREEN</code>, or <code>BLUE</code>. <P>The
     * default mode is <code>BRI</code>, because that provides the most
    ...
    /**
     * Sets the current color of this
     * <code>ColorPicker</code>. This method simply calls
     * <code>setRGB()</code> and
     ...
     * selected. <P>This is equivalent to: <BR><code>int[] i = getRGB();</code>
    ...
    /**
     * This returns the panel with several rows of spinner controls. <P>Note you
     * can also call methods such as
     * <code>setRGBControlsVisible()</code> to adjust which controls are
     * showing. <P>(This returns the panel this
     * <code>ColorPicker</code> uses, so if you put it in another container, it
    ...
    /**
     * This shows or hides the RGB spinner controls. <P>Note these live inside
     * the "expert controls", so if
     * <code>setExpertControlsVisible(false)</code> has been called, then
    ...
    /**
     * This shows or hides the HSB spinner controls. <P>Note these live inside
     * the "expert controls", so if
     * <code>setExpertControlsVisible(false)</code> has been called, then
    ...
    /**
     * This shows or hides the alpha controls. <P>Note the alpha spinner live
     * inside the "expert controls", so if
