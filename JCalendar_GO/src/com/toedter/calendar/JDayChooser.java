 ...
 *  MERCHANTABILITYsundayFore or FITNESS FOR A PARTICULAR PURPOSE.  See the
                                                   ...
                                                   FocusListener
{
    ...
    protected Color dayOutsideMonthBackground;
    protected Color selectedColor;
    protected Color sundayForeground;
    protected Color weekdayForeground;
    protected Color decorationBackgroundColor;
    protected String[] dayNames = new String[] {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
    ...
    protected boolean monthChange = false;
     ...
     */
    public JDayChooser(JMonthChooser monthChooser, JYearChooser yearChooser)
    {
        this (false, monthChooser, yearChooser);
    }
     ...
     */
    public JDayChooser(boolean weekOfYearVisible, JMonthChooser monthChooser, JYearChooser yearChooser)
    {
        setName ("JDayChooser");
    ...
    protected void init()
    {
        oldDayBackgroundColor = new Color (246, 245, 245);
        dayOutsideMonthBackground = new Color (220, 220, 220);
    ...
    /**
     * Set the day names array.
     *
     * @param dayNames The short names for weekdays.
     */
    ...
    public void drawDayNames()
    {
        for (int i = 0; i < 7; i++)
        {
            ...
            {
                if (dayNames[i].length () >= maxDayCharacters)
                {
                    dayNames[i] = dayNames[i].substring (0, maxDayCharacters);
                }
            }
    ...
    public void drawDays()
    {
        ...
        tempCal.setTime (tmpCalendar.getTime ());
        tempCal.add (Calendar.DAY_OF_MONTH, -1);
        int minDay = tempCal.get (Calendar.DAY_OF_MONTH) - firstDay + 1;
        for (i = 0; i < firstDay; i++)
        {
            days[i + 7].setText (minDay + "");
            days[i + 7].setVisible (true);
            days[i + 7].setBackground (dayOutsideMonthBackground);
            days[i + 7].setForeground (new Color (120, 120, 120));
            ++minDay;
        }
        ...
        while (day.before (firstDayInNextMonth))
        {
            ...
            {
                days[i + n + 7].setForeground (todayColor);
            }
            else
            {
        ...
        for (int k = n + i + 7; k < 49; k++)
        {
            if (k < end)
            {
                ...
                days[k].setBackground (dayOutsideMonthBackground);
                days[k].setForeground (new Color (120, 120, 120));
                ++d;
            }
            else
            {
     ...
     *
     * @param dayNum The day of the month to set as busy.
     * @param foreground The color to set the foreground to.
     */
    public void setDayForeground(int dayNum, Color foreground)
    {
        Calendar tmpCalendar = (Calendar) calendar.clone ();
        tmpCalendar.set (Calendar.HOUR_OF_DAY, 0);
        tmpCalendar.set (Calendar.MINUTE, 0);
        ...
        for (i = 0; i < firstDay; i++)
        {
            ...
            if (incr == backDayNum)
            {
                days[i + 7].setForeground (foreground);
            }
        }
        ...
        for (int k = n + i + 7; k < end; k++)
        {
            if (k < end)
            {
                if (k == dayNum)
                {
                    days[k].setForeground (foreground);
                }
            }
        }
    }
    ...
    /**
     * Reverts all day foregrounds back to their default colors.
     */
    public void revertAllDayForegrounds()
    {
        Calendar tmpCalendar = (Calendar) calendar.clone ();
        tmpCalendar.set (Calendar.HOUR_OF_DAY, 0);
        tmpCalendar.set (Calendar.MINUTE, 0);
    ...
    public void setDay(int d)
    {
        ...
        for (int i = 7; i < 49; i++)
        {
            if (days[i].getBackground () == oldDayBackgroundColor
                && days[i].getText ().equals (Integer.toString (day)))
            {
                selectedDay = days[i];
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        int localDay = new Integer (buttonText).intValue ();
        if (button.getBackground () != dayOutsideMonthBackground)
        {
            setDay (localDay);
        }
        else
        {
            ...
            if (localDay > 7)
            {
                firePropertyChange ("month", month, month - 1);
                setMonth (month - 1);
            }
            else
            {
            ...
            setDay (localDay);
            Calendar tmpCalendar = (Calendar) calendar.clone ();
            tmpCalendar.set (Calendar.DAY_OF_MONTH, localDay);

            firePropertyChange ("date", oldDate, tmpCalendar.getTime ());
            firePropertyChange ("refreshBusy", null, null);
            monthChange = false;
        }
    }
    ...
    {
        int offset = (e.getKeyCode () == KeyEvent.VK_UP) ? (-7) : ((e.getKeyCode () == KeyEvent.VK_DOWN) ? (+7)
